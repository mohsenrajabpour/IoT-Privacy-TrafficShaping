...
int
BufferTime::configure(Vector<String> &conf, ErrorHandler* errh)
{
  ...
  //&&
     ...
     if (Args(conf, this, errh)
        ...
        if(filename == "NONE" || delay <= 0 || (size1 == 0 && randomness == false) ){
            return(errh->error("File name, delay and packet size or randomness needs to be set!"));
        }
        //&&&&&
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
int
BufferTime::initialize(ErrorHandler *) {
        _timer.initialize(this);
        return 0;
}
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
Packet *
BufferTime::simple_action(Packet *p)
{
    if (!_active)
        return p;
    ...
    StringAccum sa(_label.length() + 2 // label:
                   ...
                   + 3 * bytes);
    if (sa.out_of_memory()) {
        click_chatter("no memory for BufferTime");
        return p;
    }
